...
export const userIntegration = pgTable("UserIntegration", {
	id: uuid().defaultRandom().primaryKey().notNull(),
	userId: uuid().notNull(),
	integrationName: varchar({ length: 100 }).notNull(),
	connectionId: varchar({ length: 255 }),
	authConfigId: varchar({ length: 255 }),
	isConnected: boolean().default(false).notNull(),
	connectedAt: timestamp({ mode: 'string' }),
	disconnectedAt: timestamp({ mode: 'string' }),
	metadata: json(),
	createdAt: timestamp({ mode: 'string' }).defaultNow().notNull(),
	updatedAt: timestamp({ mode: 'string' }).defaultNow().notNull(),
	authRequestId: varchar({ length: 255 }),
	authRedirectUrl: text(),
	authExpiresAt: timestamp({ mode: 'string' }),
},
(table) => {
	return {
		userIntegrationUserIdUserIdFk: foreignKey({
			columns: [table.userId],
			foreignColumns: [user.id],
			name: "UserIntegration_userId_User_id_fk"
		}).onDelete("cascade"),
		userIntegrationUserIdIntegrationNameUnique: unique("user_integration_user_id_integration_name_unique").on(table.userId, table.integrationName),
	}
});
